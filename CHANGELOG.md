...
- 启动时自动清理卡住的任务：App 启动后将 `pending`/`running`/`executing` 状态的任务标记为 `failed`（"Process exited"），同时将 `uploaded_files.parse_status = 'parsing'` 的文件重置为 `failed`，避免进程退出后任务和文件状态卡住无法删除
